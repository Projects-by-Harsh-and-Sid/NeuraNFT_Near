...
contract("CollectionContract", accounts => {
  ...
  it("should create a collection", async () => {
    const result = await collectionContract.createCollection("BaseModel1", "ipfs://image", "Test Collection", { from: user1 });
  ...
  it("should update a collection", async () => {
    const createResult = await collectionContract.createCollection("BaseModel1", "ipfs://image1", "Test Collection", { from: user1 });
    const collectionId = createResult.logs[0].args.collectionId.toNumber();

    await collectionContract.updateCollection(collectionId, "BaseModel2", "ipfs://image2", "Updated Collection", { from: user1 });
  ...
  it("should transfer collection ownership", async () => {
    const createResult = await collectionContract.createCollection("BaseModel1", "ipfs://image", "Test Collection", { from: user1 });
    const collectionId = createResult.logs[0].args.collectionId.toNumber();
  ...
  it("should get total collections", async () => {
    await collectionContract.createCollection("BaseModel1", "ipfs://image1", "Test Collection 1", { from: user1 });
    await collectionContract.createCollection("BaseModel2", "ipfs://image2", "Test Collection 2", { from: user1 });
  ...
  it("should not allow non-owners to update collection", async () => {
    ...
    await truffleAssert.reverts(
      collectionContract.updateCollection(collectionId, "BaseModel2", "ipfs://image2", "Updated Collection", { from: user2 }),
